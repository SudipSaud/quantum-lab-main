...
async def _llm_playground(prompt: str) -> PlaygroundResponse | None:
    sys = (
...
async def ai_chat(body: ChatRequest) -> ChatResponse:
    ...
    try:
        raw = await _call_openai_compatible(msgs)
...
async def ai_playground(body: PlaygroundRequest) -> PlaygroundResponse:
    try:
        llm = await _llm_playground(body.prompt)
        if llm:
